...
或者，启动支持GPU的容器（要求必须执行了Step3）：

```bash
docker run -it --name ESfunfuzz -p 3821:3821 --gpus all ty_esfunfuzz:3.0
```

通常，启动容器后会自动进入该容器，即可开始使用。但假如你的Docker并未进入该容器，则需要再使用如下命令手动进入容器：
...
...
[2021-05-13T09:16:13.790] [INFO] run - 解析成功，结果文件存放在 /root/ESfunfuzz/CaseMutator/StandardParser/resources/result_new.json
```
...
训练过程可以调整更多参数，详情参考 `/root/ESfunfuzz/CodeGenerator/conf.py` 配置文件。 
...
启动后，可以使用浏览器访问 `本机ip:3821` 访问部署好的Jupyter演示笔记，比如 `10.15.0.88:3821` （注意ip需要换成你本机的），访问密码为 `nisl8830` ；进入Jupyter笔记后可通过 `main.ipynb` 进行演示。
